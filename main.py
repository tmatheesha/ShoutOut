...
def print_define(results, target, metadata=None):
    ...
    for result in results["results"]["bindings"]:
        if result[target]["xml:lang"] == "en":
            ...
            #print
            tests.append(result[target]["value"])
...
def answerQuestion(question):
    ...
    for question in questions:
        ...
        # a = print_handlers[query_type](results, target, metadata)
        output_list= print_define(results,target,metadata=None)
        #print output_list
        output_str=''.join(output_list).encode('utf-8')
        return output_str